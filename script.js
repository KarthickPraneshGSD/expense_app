...
// Note: synchronizeTabs is intentionally omitted to avoid the deprecated
// enableMultiTabIndexedDbPersistence() API. Single-tab persistence is used instead.
db.enablePersistence().catch(err => {
  if (err.code === 'failed-precondition') {
    console.warn('Offline persistence disabled: close other tabs and reload.');
...
async function adminChangePassword() {
  // Changing another user's Firebase Auth password requires the Admin SDK (server-side).
  // This is a client-only app, so we inform the admin.
  notify('⚠️ To reset a user password, ask them to use "Forgot Password" or set up Firebase Admin SDK via Cloud Functions.', 'info');
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (filterDateInput) filterDateInput.addEventListener('change', renderExpenses);
